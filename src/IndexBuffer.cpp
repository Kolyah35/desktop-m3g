...
IndexBuffer::IndexBuffer(int type_, int num_indices, const int* indices, int num_lengths, const int* lengths) : type(type_), gl(0) {
    if (type != TRIANGLES) {
...
IndexBuffer::IndexBuffer(int type_, int first_index, int num_lengths, const int* lengths) : type(type_), gl(0) {
    if (type != TRIANGLES) {
        throw IllegalArgumentException(__FILE__, __func__, "Primitive type is invalid, type=%d.", type);
    }
    if (lengths == NULL) {
...
IndexBuffer* IndexBuffer::duplicate_xxx(Object3D* obj) const {
    ...
    if (ibuf == NULL) {
        ibuf = new IndexBuffer(type, indices_size, indices, lengths_size, lengths);
    }
...
int IndexBuffer::getIndexCount() const {
    ...
    switch (type) {
        ...
        case LINES:
        case POINT_SPRITES:
        default: throw InternalException(__FILE__, __func__, "Primitive type is invalid, type=%d.", type);
    };
    return num;
}
...
void IndexBuffer::render_xxx(RenderState& state) const {
    ...
    long long offset = 0;
    for (int i = 0; i < (int)strip_lengths.size(); i++) {
        glDrawElements(GL_TRIANGLE_STRIP, strip_lengths[i], INDEX_TYPE, (GLvoid*)offset);
