...
Mesh::Mesh(VertexBuffer* vertices_, int num_submesh, IndexBuffer** submeshes, int num_appearance, Appearance** appearances_) : vertices(NULL) {
    if (vertices_ == NULL) {
...
Mesh* Mesh::duplicate_xxx(Object3D* obj) const {
    ...
    if (mesh == NULL) {
        mesh = new Mesh(vertices, indices.size(), (IndexBuffer**)&indices[0], appearances.size(), (Appearance**)&appearances[0]);
    }
...
bool Mesh::intersect(const Vector& org, const Vector& dir, RayIntersection* ri_out) const {
    ...
    if (ri_hit) {
        // cout << "ri_hit = " << ri_hit << "\n";
        if (ri_out) {
            *ri_out = RayIntersection(const_cast<Mesh*>(this), org, dir, ri_t, ri_u, ri_v, 3, ri_index_values, ri_submesh_index);
        }
        return true;
    }
