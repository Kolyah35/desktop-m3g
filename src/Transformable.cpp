...
Transformable::Transformable() : scaling(1, 1, 1), translation(0, 0, 0), orientation(0, 0, 0, 1) {}
...
Transformable* Transformable::duplicate_xxx(Object3D* obj) const {
    Transformable* trans = dynamic_cast<Transformable*>(obj);
    if (trans == NULL) {
        trans = new Transformable;
    }
    Object3D::duplicate_xxx(trans);
...
void Transformable::addAnimationTrack_xxx(AnimationTrack* animation_track, bool accepted) {
    ...
    if (property == AnimationTrack::ORIENTATION || property == AnimationTrack::SCALE || property == AnimationTrack::TRANSLATION) {
        accepted = true;
    }
...
void Transformable::render_xxx(RenderState& state) const {
    Transform trans;
