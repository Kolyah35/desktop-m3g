...
Fog::Fog() : mode(LINEAR), color(0), density(1), distance(0, 1) {}
...
void Fog::addAnimationTrack_xxx(AnimationTrack* animation_track, bool accepted) {
    ...
    if (property == AnimationTrack::COLOR || property == AnimationTrack::DENSITY || property == AnimationTrack::FAR_DISTANCE || property == AnimationTrack::NEAR_DISTANCE) {
        accepted = true;
    }
...
void Fog::render_xxx(RenderState& state) const {
    ...
    GLfloat rgb[3] = {((color & 0x00ff0000) >> 16) / 255.f, ((color & 0x0000ff00) >> 8) / 255.f, ((color & 0x000000ff) >> 0) / 255.f};
    ...
    switch (mode) {
        ...
        case LINEAR:
            ...
            glFogfv(GL_FOG_COLOR, rgb);
            break;
        default: throw InternalException(__FILE__, __func__, "Fog mode is unknown, mode=%d.", mode);
    }
}
