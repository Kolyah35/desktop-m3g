...
Image2D::Image2D(int format_, int width_, int height_) : format(format_), width(width_), height(height_), image(0), immutable(false) {
    if (width <= 0 || height <= 0) {
        throw IllegalArgumentException(__FILE__, __func__, "Width or height is invalid, width=%f, height=%f.", width, height);
    }
    if (format != Image2D::ALPHA && format != Image2D::LUMINANCE && format != Image2D::LUMINANCE_ALPHA && format != Image2D::RGB && format != Image2D::RGBA) {
...
Image2D::Image2D(int format_, int width_, int height_, void* pixels) : format(format_), width(width_), height(height_), image(0), immutable(true) {
    if (width <= 0 || height <= 0) {
        throw IllegalArgumentException(__FILE__, __func__, "Width or height is invalid, width=%f, height=%f.", width, height);
    }
    if (pixels == NULL) {
...
Image2D::Image2D(int format_, int width_, int height_, unsigned char* pixels, void* palette) : format(format_), width(width_), height(height_), image(0), immutable(true) {
