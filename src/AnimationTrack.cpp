...
AnimationTrack::AnimationTrack(KeyframeSequence* sequence_, int property_) : property(0), keyframe_sequence(0), animation_controller(0) {
    if (sequence_ == NULL) {
    ...
    if (!is_property_has_valid_component(property_, sequence_->getComponentCount())) {
        throw IllegalArgumentException(__FILE__, __func__, "Component count of KeyframeSequence is invalid for this property, count=%d, property=%d",
                                       sequence_->getComponentCount(), property_);
    }
...
static const char* property_to_string(int property) {
    switch (property) {
