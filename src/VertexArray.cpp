...
VertexArray::VertexArray(int num_vertices, int num_components, int component_size_)
    : component_count(num_components), component_size(component_size_), vertex_count(num_vertices), char_values(0), vbo(0) {
...
void VertexArray::get(int first_vertex, int num_vertices, char* values) const {
    ...
    memcpy((char*)values, (char*)&char_values[first_vertex * component_count], num_vertices * component_count * sizeof(char));
}
...
void VertexArray::get(int first_vertex, int num_vertices, short* values) const {
    ...
    memcpy((char*)values, (char*)&short_values[first_vertex * component_count], num_vertices * component_count * sizeof(short));
}
...
void VertexArray::get(int first_vertex, int num_vertices, float* values) const {
    ...
    memcpy((char*)values, (char*)&float_values[first_vertex * component_count], num_vertices * component_count * sizeof(float));
}
...
void VertexArray::get(int first_vertex, int num_vertices, float scale, float* bias, float* values) const {
    ...
    switch (component_size) {
        ...
        case 4:
            ...
            break;
        default: throw IllegalStateException(__FILE__, __func__, "Comonent size is invalid, size=%d.", component_size);
    }
}
...
void VertexArray::set(int first_vertex, int num_vertices, const char* values) {
    ...
    memcpy((char*)&char_values[offset], (char*)values, size);

    glBindBuffer(GL_ARRAY_BUFFER, vbo);
    glBufferSubData(GL_ARRAY_BUFFER, offset_size, size, &char_values[offset]);
...
void VertexArray::set(int first_vertex, int num_vertices, const short* values) {
    ...
    memcpy((char*)&short_values[offset], (char*)values, size);

    glBindBuffer(GL_ARRAY_BUFFER, vbo);
    glBufferSubData(GL_ARRAY_BUFFER, offset_size, size, &short_values[offset]);
...
void VertexArray::set(int first_vertex, int num_vertices, const float* values) {
    ...
    memcpy((char*)&float_values[offset], (char*)values, size);

    glBindBuffer(GL_ARRAY_BUFFER, vbo);
    glBufferSubData(GL_ARRAY_BUFFER, offset_size, size, &float_values[offset]);
...
void VertexArray::setMorphing(const VertexArray* base, const std::vector<const VertexArray*>& targets, const std::vector<float>& weights) {
    if (base == NULL) {
    ...
    for (int v = 0; v < vertex_count; v++) {
        ...
        switch (base->component_size) {
            case 1: v2 = v0 = Vector(base->char_values[v * 3], base->char_values[v * 3 + 1], base->char_values[v * 3 + 2]); break;
            case 2: v2 = v0 = Vector(base->short_values[v * 3], base->short_values[v * 3 + 1], base->short_values[v * 3 + 2]); break;
            case 4: v2 = v0 = Vector(base->float_values[v * 3], base->float_values[v * 3 + 1], base->float_values[v * 3 + 2]); break;
        }
        for (int t = 0; t < (int)targets.size(); t++) {
            Vector v1;
            switch (targets[t]->component_size) {
                case 1: v1 = Vector(targets[t]->char_values[v * 3], targets[t]->char_values[v * 3 + 1], targets[t]->char_values[v * 3 + 2]); break;
                case 2: v1 = Vector(targets[t]->short_values[v * 3], targets[t]->short_values[v * 3 + 1], targets[t]->short_values[v * 3 + 2]); break;
                case 4: v1 = Vector(targets[t]->float_values[v * 3], targets[t]->float_values[v * 3 + 1], targets[t]->float_values[v * 3 + 2]); break;
            }
            v2 += weights[t] * (v1 - v0);
        }
        for (int i = 0; i < 3; i++) {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, vbo);
    glBufferSubData(GL_ARRAY_BUFFER, 0, vertex_count * component_count * component_size, char_values);
...
void VertexArray::setSkinning(const VertexArray* base_positions, const std::vector<std::vector<BoneWeight>>& bone_indices, const std::vector<Matrix>& matrix_palette) {
    ...
    for (int v = 0; v < vertex_count; v++) {
        ...
        switch (base_positions->component_size) {
            case 1: v0 = Vector(base_positions->char_values[v * 3], base_positions->char_values[v * 3 + 1], base_positions->char_values[v * 3 + 2]); break;
            case 2: v0 = Vector(base_positions->short_values[v * 3], base_positions->short_values[v * 3 + 1], base_positions->short_values[v * 3 + 2]); break;
            case 4: v0 = Vector(base_positions->float_values[v * 3], base_positions->float_values[v * 3 + 1], base_positions->float_values[v * 3 + 2]); break;
...
void VertexArray::updateOpenGLData(const void* values) const {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, vbo);
    glBufferSubData(GL_ARRAY_BUFFER, 0, vertex_count * component_count * component_size, values);
    ...
    switch (from) {
        case 1: {
            switch (to) {
                case 1:
                    for (int i = 0; i < num; i++) {
                        target_char_values[i] = char_values[i];
                    }
                    break;
                case 2:
                    for (int i = 0; i < num; i++) {
                        target_short_values[i] = char_values[i];
                    }
                    break;
                case 4:
                    for (int i = 0; i < num; i++) {
                        target_float_values[i] = char_values[i];
                    }
                    break;
            }
            break;
        ...
        case 2: {
            switch (to) {
                case 1:
                    for (int i = 0; i < num; i++) {
                        target_char_values[i] = short_values[i];
                    }
                    break;
                case 2:
                    for (int i = 0; i < num; i++) {
                        target_short_values[i] = short_values[i];
                    }
                    break;
                case 4:
                    for (int i = 0; i < num; i++) {
                        target_float_values[i] = short_values[i];
                    }
                    break;
            }
            break;
        ...
        case 4: {
            switch (to) {
                case 1:
                    for (int i = 0; i < num; i++) {
                        target_char_values[i] = float_values[i];
                    }
                    break;
                case 2:
                    for (int i = 0; i < num; i++) {
                        target_short_values[i] = float_values[i];
                    }
                    break;
                case 4:
                    for (int i = 0; i < num; i++) {
                        target_float_values[i] = float_values[i];
                    }
                    break;
            }
            break;
