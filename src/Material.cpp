...
Material::Material() : vertex_color_tracking(false), ambient_color(0x00333333), diffuse_color(0xffcccccc), emissive_color(0), specular_color(0), shininess(0) {}
...
void Material::addAnimationTrack_xxx(AnimationTrack* animation_track, bool accepted) {
    ...
    if (property == AnimationTrack::ALPHA || property == AnimationTrack::AMBIENT_COLOR || property == AnimationTrack::DIFFUSE_COLOR || property == AnimationTrack::EMISSIVE_COLOR ||
        property == AnimationTrack::SHININESS || property == AnimationTrack::SPECULAR_COLOR) {
        accepted = true;
    }
...
void Material::render_xxx(RenderState& state) const {
    ...
    GLfloat ambient_rgb[4] = {((ambient_color & 0x00ff0000) >> 16) / 255.f, ((ambient_color & 0x0000ff00) >> 8) / 255.f, ((ambient_color & 0x000000ff) >> 0) / 255.f, 1};
    glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, ambient_rgb);

    GLfloat diffuse_rgba[4] = {((diffuse_color & 0x00ff0000) >> 16) / 255.f, ((diffuse_color & 0x0000ff00) >> 8) / 255.f, ((diffuse_color & 0x000000ff) >> 0) / 255.f,
    ...
    GLfloat specular_rgb[4] = {((specular_color & 0x00ff0000) >> 16) / 255.f, ((specular_color & 0x0000ff00) >> 8) / 255.f, ((specular_color & 0x000000ff) >> 0) / 255.f, 1};
    glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, specular_rgb);
    glMaterialf(GL_FRONT_AND_BACK, GL_SHININESS, shininess);

    GLfloat emissive_rgb[4] = {((emissive_color & 0x00ff0000) >> 16) / 255.f, ((emissive_color & 0x0000ff00) >> 8) / 255.f, ((emissive_color & 0x000000ff) >> 0) / 255.f, 1};
    ...
    if (vertex_color_tracking) {
        ...
        if (state.vertex_color_buffer_ready) {
            ...
            GLfloat vertex_color_rgba[4] = {((state.default_vertex_color & 0x00ff0000) >> 16) / 255.f, ((state.default_vertex_color & 0x0000ff00) >> 8) / 255.f,
                                            ((state.default_vertex_color & 0x000000ff) >> 0) / 255.f, ((state.default_vertex_color & 0xff000000) >> 24) / 255.f * state.alpha};
