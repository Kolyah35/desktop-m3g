...
namespace {
    // スプライト画像の表示に使う板ポリゴン.
    float positions[12] = {0.5, 0.5, 0, 0.5, -0.5, 0, -0.5, 0.5, 0, -0.5, -0.5, 0};

    float tex_coords[8] = {1, 0, 1, 1, 0, 0, 0, 1};
    unsigned short indices[4] = {0, 1, 2, 3};
} // namespace

Sprite3D::Sprite3D(bool scaled_, Image2D* image_, Appearance* appearance_) : scaled(false), image(0), appearance(0), crop(0, 0, 0, 0), gl(0, 0, 0, 0) {
    if (image_ == NULL) {
...
bool Sprite3D::intersect(const Vector& org, const Vector& dir, const Camera* cam, RayIntersection* ri) const {
    ...
    for (int i = 0; i < 2; i++) {
        hit = triangle_intersect(org, dir, *vertices[i][0], *vertices[i][1], *vertices[i][2], &u, &v, &t);
        if (hit) {
            if (ri) {
                *ri = RayIntersection(const_cast<Sprite3D*>(this), org, dir, t, u, v, 3, indices[i], 0);
            }
            return true;
        }
    }
