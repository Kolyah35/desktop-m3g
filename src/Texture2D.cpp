...
Texture2D::Texture2D(Image2D* img)
    : image(0), wrapping(WRAP_REPEAT, WRAP_REPEAT), filter(FILTER_BASE_LEVEL, FILTER_NEAREST), blending_mode(FUNC_MODULATE), blend_color(0x00000000), texobj(0) {
    if (img == NULL) {
...
void Texture2D::render_xxx(RenderState& state) const {
    ...
    GLfloat color[4] = {((blend_color && 0x00ff0000) >> 16) / 255.f, ((blend_color && 0x0000ff00) >> 8) / 255.f, ((blend_color && 0x000000ff) >> 0) / 255.f,
                        ((blend_color && 0xff000000) >> 24) / 255.f};

    switch (blending_mode) {
        case FUNC_ADD: glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_ADD); break;
        case FUNC_BLEND:
            glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_BLEND);
            glTexEnvfv(GL_TEXTURE_ENV, GL_TEXTURE_ENV_COLOR, color);
            break;
        case FUNC_DECAL: glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_DECAL); break;
        case FUNC_MODULATE: glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE); break;
        case FUNC_REPLACE: glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE); break;
...
static const char* wrap_to_string(int wrap) {
    switch (wrap) {
...
static const char* filter_to_string(int filter) {
    switch (filter) {
...
static const char* blending_to_string(int blending) {
    switch (blending) {
