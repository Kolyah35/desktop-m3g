 ...
 *       それの先取り。
 */
TriangleStripArray::TriangleStripArray(int num_indices, const int* indices, int num_lengths, const int* lengths)
    : IndexBuffer(IndexBuffer::TRIANGLES, num_indices, indices, num_lengths, lengths) {}

TriangleStripArray::TriangleStripArray(int first_index, int num_lengths, const int* lengths) : IndexBuffer(IndexBuffer::TRIANGLES, first_index, num_lengths, lengths) {}
...
TriangleStripArray* TriangleStripArray::duplicate_xxx(Object3D* obj) const {
    ...
    if (tris == NULL) {
        tris = new TriangleStripArray(indices_size, indices, lengths_size, lengths);
    }
