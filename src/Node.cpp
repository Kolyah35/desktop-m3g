...
Node::Node() : parent(0), rendering_enable(true), picking_enable(true), alpha_factor(1), scope(-1), z_alignment(NONE, 0), y_alignment(NONE, 0), duplicated(0) {}
...
void Node::addAnimationTrack_xxx(AnimationTrack* animation_track, bool accepted) {
    ...
    if (property == AnimationTrack::ALPHA || property == AnimationTrack::PICKABILITY || property == AnimationTrack::VISIBILITY) {
        accepted = true;
    }
...
void Node::setAlignment(Node* z_reference, int z_target, Node* y_reference, int y_target) {
    if (!(z_target == NONE || z_target == ORIGIN) || !(y_target == NONE || y_target == ORIGIN)) {
