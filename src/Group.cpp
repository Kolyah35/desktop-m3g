...
Group* Group::duplicate_xxx(Object3D* obj) const {
    Group* grp = dynamic_cast<Group*>(obj);
    if (grp == NULL) {
        grp = new Group;
    }
    Node::duplicate_xxx(grp);
...
bool Group::pick(int scope, float x, float y, const Camera* camera, RayIntersection* ri_out) const {
    ...
    for (int i = 0; i < (int)children.size(); i++) {
        ...
        if (ri.getIntersected()) {
            if (min_ri.getIntersected() == NULL || ri.getDistance() < min_ri.getDistance()) {
                min_ri = ri;
            }
        }
    }
...
bool Group::pick(int scope, float ox, float oy, float oz, float dx, float dy, float dz, RayIntersection* ri_out) const {
    ...
    for (int i = 0; i < (int)children.size(); i++) {
        ...
        if (grp2) {
            ...
            grp2->pick(scope, org.x, org.y, org.z, dir.x, dir.y, dir.z, &ri);
            // レイはこのGroupの座標系で格納する
            trans.invert();
            ri.transformRay(trans);
        }
