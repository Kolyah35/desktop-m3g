...
Camera::Camera() : type(GENERIC), fovy(0), aspect_ratio(0), near(0), far(0), projection() {}
...
void Camera::addAnimationTrack_xxx(AnimationTrack* animation_track, bool accepted) {
    ...
    if (property == AnimationTrack::FAR_DISTANCE || property == AnimationTrack::FIELD_OF_VIEW || property == AnimationTrack::NEAR_DISTANCE) {
        accepted = true;
    }
...
void Camera::lookAt(float from_x, float from_y, float from_z, float to_x, float to_y, float to_z, float up_x, float up_y, float up_z) {
    if (from_x == to_x && from_y == to_y && from_z == to_z) {
        throw IllegalArgumentException(__FILE__, __func__, "Viewing from(%f,%f,%f) - to(%f,%f,%f) is invalied.", from_x, from_y, from_z, to_x, to_y, to_z);
    }
    ...
    float m[16] = {right.x, up.x, -view.x, from.x, right.y, up.y, -view.y, from.y, right.z, up.z, -view.z, from.z, 0, 0, 0, 1};
    Transform trans;
    trans.set(m);
