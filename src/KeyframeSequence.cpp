...
KeyframeSequence::KeyframeSequence(int num_keyframes, int num_components, int interpolation)
    : repeat_mode(CONSTANT), interp_type(LINEAR), duration(0), valid_range(-1, -1), component_count(0), keyframe_count(0) {
    if (num_keyframes < 1) {
...
KeyframeSequence* KeyframeSequence::duplicate_xxx(Object3D* obj) const {
    ...
    for (int i = 0; i < keyframe_count; i++) {
        ...
        memcpy(key_seq->keyframes[i].value, keyframes[i].value, sizeof(float) * component_count);
    }
...
void KeyframeSequence::setKeyframe(int index, int time, const float* value) {
    ...
    memcpy(keyframes[index].value, value, sizeof(float) * component_count);
}
...
void KeyframeSequence::sample(int sequence_time, float* value) const {
    ...
    const Keyframe& k0 = (left == first && repeat_mode == LOOP) ? Keyframe(-1, 0) : (left == first && repeat_mode == CONSTANT) ? Keyframe(-1, 0) : keyframes[left - 1];
    const Keyframe& k1 = keyframes[left];
    const Keyframe& k2 = keyframes[right];
    const Keyframe& k3 = (right == last && repeat_mode == LOOP) ? Keyframe(-1, 0) : (right == last && repeat_mode == CONSTANT) ? Keyframe(-1, 0) : keyframes[right + 1];

    switch (interp_type) {
        case STEP: step(s, k1, k2, component_count, value); return;
        case LINEAR: linear(s, k1, k2, component_count, value); return;
        case SLERP: slerp(s, k1, k2, component_count, value); return;
        case SPLINE: spline(s, k0, k1, k2, k3, component_count, value); return;
        case SQUAD: squad(s, k0, k1, k2, k3, component_count, value); return;
        default: throw InternalException(__FILE__, __func__, "Interpolation type is unknown, type=%d.", interp_type);
    }
}

static const char* repeat_mode_to_string(int mode) {
    switch (mode) {
...
static const char* interp_type_to_string(int interp) {
    switch (interp) {
