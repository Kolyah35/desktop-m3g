...
M3GWriter::M3GWriter(const char* name_)
    : name(name_), stream(0), compression_scheme(false), object_index(1), object_type(0), start_of_section(0), start_of_objects(0), start_of_object(0),
      position_of_header_section(0), position_of_header(0), position_of_header_checksum(0) {
...
M3GWriter::~M3GWriter() {
    if (position_of_header) {
        ...
        unsigned int checksum = adler32(init_value, stream->pointer(position_of_header_section), position_of_header_checksum - position_of_header_section);
...
void M3GWriter::endSection() {
    ...
    if (compression_scheme == 1) {
        ...
        int ret;
        ret = compress((Bytef*)buf, (uLongf*)&compressed_length, (const Bytef*)stream->pointer(start_of_objects), objects_length);
        assert(ret == Z_OK);
    ...
    unsigned int init_value = adler32(0, NULL, 0);
    unsigned int checksum = adler32(init_value, stream->pointer(start_of_section), section_length - 4);
    putUInt32(checksum);
}
